...
<h4 align="center">
  Browse • Map • Discover • Capture • Retrieve
</h4>
...
- **Browsing Defences**: trackers and third-party cookies blocked, click identifiers stripped, AI-free search results, and opt-in proxy and timezone pinning. Not anonymity — see [Privacy Ice Wall](#privacy-ice-wall).
...
Browsing itself is defended too, though less completely — this part is a set of
specific measures, not a guarantee:

- Tracker and ad requests are blocked before they leave (macOS, Linux, Windows).
- Third-party cookies are blocked on macOS, Linux and Android. **Not on Windows** —
  WebView2 has no equivalent, and the app says so on the Settings screen.
- Click identifiers (`utm_*`, `gclid`, `fbclid`, …) are stripped both on navigation
  and on capture, so an ad attribution never gets a permanent home in your library.
- Searches ask the engine for results without AI-generated answers, where the engine
  offers a way to. On by default.
- **Optional, off by default:** route all traffic through a SOCKS5/HTTP proxy such as
  Tor, and report UTC and a fixed locale to pages instead of your own timezone.

What that does **not** add up to is anonymity. Sites still see your TLS fingerprint
and the usual canvas, WebGL and font fingerprinting surface, and ÆTHER cannot change
that without shipping its own engine. A hidden IP with an unchanged fingerprint still
...
- AI-free search results, on by default — asks the selected engine to skip its
  AI-generated answer where it offers a way to. The screen names the mechanism for
  the engine you have chosen, and says so plainly when that engine offers none.
- Route traffic through a proxy, off by default. One SOCKS5 or HTTP CONNECT
  endpoint for both tabs and the app's own fetches, prefilled with Tor's
  `socks5://127.0.0.1:9050`. Needs macOS 14+; unavailable on Android.
- Report UTC to websites, off by default — reports UTC and `en-US` in place of your
  timezone and language. Local times on sites will read wrong; that is the trade.
- Appearance (system, light, dark) and automatic update checks.
- Clear browsing data — cookies, caches and site storage.
- Developer Mode, which exposes more advanced/experimental funtionality.
- Keyboard shortcut reference.

The proxy and timezone settings apply to tabs opened _after_ the change: both are
fixed when a webview is created, so existing tabs keep what they were born with.
...
**Commercial licensing:** write to **canpixeldev@gmail.com**. A commercial
license is available and priced per case.

**Contributions are welcome, and you do not need to ask first.**
[`CONTRIBUTING.md`](CONTRIBUTING.md) carries a standing grant that lets anyone
fork and modify ÆTHER for the purpose of contributing to it, and lets you patch
your own copy for your own use. That grant deliberately does **not** extend to
